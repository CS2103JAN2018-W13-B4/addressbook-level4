...
* Aiding the `Logic` component by switching the month view accordingly when users execute the respective calendar feature commands (e.g. `pmonth`, `nmonth`, etc.). A calendar feature command is defined as a command that changes the display of the calendar. For the full list of calendar feature commands, head to <<UserGuide#_calendar_features, User Guide: Calendar Features>>.
